...
std::string DSON_Node::GetValueOf(std::string IN_Name) {
    if (IN_Name.length() == 0) { return "{#NULL}"; }

    std::vector<std::string> Parts = DSON::SplitString(IN_Name,'/');
	UInt I = GetIndexOf(Parts[0]);
	if (I == DSON_UINT_MAX) { return "{#NULL}"; }

	UInt Num = Parts.size();
	if (Num == 1) {
		if (Childs[I]->isValue()) { return Childs[I]->Value; }
		else { return "{#GROUP}"; }
	}
	...
	return Childs[I]->GetValueOf(NewValue);
}


DSON_Node* DSON_Node::GetNodeOf(std::string IN_Name) {
    if (IN_Name.length() == 0) { return nullptr; }

    std::vector<std::string> Parts = DSON::SplitString(IN_Name,'/');
	UInt I = GetIndexOf(Parts[0]);
	if (I == DSON_UINT_MAX) { return nullptr; }
...
bool DSON_Node::CheckAndGetValueOf(std::string IN_Name, std::string& OUT_Value) {
    OUT_Value = "";

	std::string Temp = GetValueOf(IN_Name);
...
std::string DSON_Tree::GetValueOf(std::string IN_Name) {
	if (Root == nullptr) {
		DSON::Log_Error("GetValueOf() faile, DSON_Tree => RootNode is NULLPTR");
		return "";
	}
	return Root->GetValueOf(IN_Name);
}
...
bool DSON_Tree::CheckAndGetValueOf(std::string IN_Name, std::string& OUT_Value) {
	if (Root == nullptr) {
		DSON::Log_Error("CheckAndGetValueOf() faile, DSON_Tree => RootNode is NULLPTR");
		return false;
	}
	return Root->CheckAndGetValueOf(IN_Name, OUT_Value);
}
...
DSON_Node* DSON_Parser::ParseFile(std::string IN_FileName) {
	...
	while (std::getline(InFile, CurrLine)) {
		...
		DSON::Log_Debug("Line parsed Successfully");
		LNum++;
	}
